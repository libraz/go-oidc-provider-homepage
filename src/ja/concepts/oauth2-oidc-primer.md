...
```mermaid
flowchart LR
  RP[Relying Party<br/>あなたのアプリ] -->|"ユーザをログインへ送る"| OP[OpenID Provider<br/>このライブラリ]
  OP -->|"code を付けて戻す"| RP
  RP -->|"code をトークンに交換"| OP
  OP -->|"ID トークン + アクセストークン"| RP
  RP -->|"アクセストークンで API 呼び出し"| RS[Resource Server<br/>あなたの API]
```

| ロール | 概要 | 役割 |
|---|---|---|
| **OP**（OpenID Provider、別名 Authorization Server） | ユーザの身元を握っている HTTP サーバ。 | ユーザ認証とトークン発行を担う。**`go-oidc-provider` がこれです。** |
| **RP**（Relying Party、別名 Client） | ユーザが誰かを知りたいアプリ。 | ログインを開始し、アクセストークンを保持し、API を呼ぶ。 |
| **RS**（Resource Server） | データを返す API。 | アクセストークンを検証し、データを返す。 |
...
OpenID Connect（OIDC）は **OAuth 2.0 + 身元** を扱う規格です。OP は加えて **ID トークン**（署名付き JWT）を発行し、「このトークンはユーザ `sub=alice123`、audience `client_id=myapp` 向けに、この時刻に発行された。Alice について次の claim が真である」と表明します。OIDC はさらに `userinfo` エンドポイント、discovery、RP-Initiated Logout、Back-Channel Logout 通知も追加します。
...
よくある落とし穴です。ID トークンの audience は RP であって RS ではありません。API に `Authorization: Bearer` で送ると技術的には通ってしまいますが、意味的には誤り — RP の機微情報（email など）をユーザが触る全 API に晒すことになります。**API にはアクセストークンを使ってください。** RFC 7662 の introspection、もしくは RFC 9068 の self-contained JWT として検証します。
:::
