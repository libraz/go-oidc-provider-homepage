...
| `Subject` | OP 内部の安定したユーザ ID。ID Token の `sub` になります。 |
| `AuthTime` | ユーザがいつ認証したか。ID Token の `auth_time` になります。 |
...
`/end_session` は単に「cookie を消す」だけではありません。組み込み側が `Grants` と `AccessTokens` の substore を組み込んでいる場合、本ライブラリは subject が保持するすべての grant を歩き、grant ごとの access token shadow row を失効させます。JWT access token は OP が応答するエンドポイント（`/userinfo`、`/introspect`）で inactive になります。opaque access token は introspection を経由するすべての RS で inactive になります。
...
セッションの substore は、トランザクショナルストア（authorization code、refresh token、client）から意図的に分離されています。組み込み側は典型的には次のいずれかを選びます。
