...
::: tip 順番に読む
OAuth と OIDC が初めての方は、ステップ 1 から順に降りていってください。多くの読者がつまづくのは仕様が難しいからではなく、「issuer とは」「redirect URI とは」を最初に固める前に、いきなり refresh token や DPoP の話に進んでしまうからです。本ページはその順序を整えるためにあります。
:::
...
実運用で 9 割を占める 3 つの grant です。記載順に読んでください。refresh token は authorization code を前提にしますし、client credentials は authorization code と比較するのが一番分かりやすいです。
...
- [Client Credentials](/ja/concepts/client-credentials) — サービス間通信。ユーザなし、refresh token なし、クライアントが自身を認証するだけ。
...
- [Access token の形式（JWT と opaque）](/ja/concepts/access-token-format) — JWT access token (RFC 9068) を選ぶべき場面、opaque を維持すべき場面、introspection のコスト。
...
- [同意](/ja/concepts/consent) — 同意画面が記録するもの、スキップできる条件、本ライブラリでのファーストパーティ クライアントのモデル化。
