---
title: 概念を学ぶ
description: 概念ページの推奨順。各ステップは前のステップで扱った内容だけを前提にしているので、上から順に読めば戻る必要はありません。
---

# 概念を学ぶ

このページは `/ja/concepts/` 配下のすべてのページに対する **推奨順** です。各ページは前のステップで扱った内容だけを前提にしているので、上から順に読めば途中で戻る必要はありません。

::: tip 順番に読む
OAuth と OIDC が初めての方は、ステップ 1 から順に読んでください。よくつまずくのは仕様が難しいからではなく、「issuer とは」「redirect URI とは」を最初に固める前に、いきなりリフレッシュトークンや DPoP の話に進んでしまうからです。本ページはその順序を整えるためにあります。
:::

::: details すでに知っている場合は飛ばして OK
- OAuth の役割分担、scope、`code` + PKCE はすでに把握している → **ステップ 4(トークン)** か **ステップ 6(送信者制約トークン)** から読み始めて構いません。
...
- [OAuth 2.0 / OIDC 入門](/ja/concepts/oauth2-oidc-primer) — 3 つの役割、それらが共有する 1 つの流れ、そしてドキュメントの残りで繰り返し出てくる略語の早見表。

## ステップ 2: 設定の基礎

以降のすべてのページが前提にしている語彙です。ここはまだ処理の流れやトークンの話ではなく、OP とそのクライアントを識別する文字列・URL・鍵素材の話です。

- [issuer / 発行者](/ja/concepts/issuer) — OP の識別 URL、`iss` claim、そしてなぜ正規形がただひとつでなければならないか。
...
実運用で大半を占める 3 つの grant です。記載順に読んでください。リフレッシュトークンは認可コードを前提にしますし、client credentials は認可コードと比較するのが一番分かりやすいです。
...
トークンが発行された後、ユーザは OP に session を持ち、何に同意したかの記録が残ります。この 2 つのページは、ほとんどの処理が残す OP 側の状態を扱います。
...
頻度の低い処理やプロファイルです。具体的な必要があるとき — TV のような機器、電話を認証器にする構成、サービス間のトークン交換、FAPI を要求する規制 — に限って読んでください。

- [Device Code（RFC 8628）](/ja/concepts/device-code) — 入力が制約されたデバイス: TV、CLI、IoT。
- [CIBA](/ja/concepts/ciba) — リクエストを発する機器にブラウザがない構成での、電話への push 認証。
