...
| `userinfo_endpoint` | `/userinfo` の URL — RP が access token を使って最新の claim を取得する場所。 |
...
| `id_token_signing_alg_values_supported` | ID Token の署名に使う JWS アルゴリズム。本ライブラリは閉じた集合 `RS256`、`PS256`、`ES256`、`EdDSA` を公開します。 |
...
OP はリクエストのたびに in-memory 状態から discovery ドキュメントを組み立てます（安価です）。コストがかかる方の話 — 直近にローテーションされた鍵を使った ID Token の JWS 検証 — こそが、discovery キャッシュの無効化が解こうとしている本当の問題です。詳細は [JWKS ローテーション](/ja/operations/key-rotation) を参照。
:::
...
- **ID Token に HMAC 系 alg を出さない。** `HS256` / `HS384` / `HS512` は広告しません。alg-confusion 攻撃クラスはこれらが到達可能であることを必要とするため、型を閉じることで攻撃を構造的に閉じます。
