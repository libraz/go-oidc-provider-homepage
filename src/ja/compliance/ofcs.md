...
スナップショット ID: `2026-05-06T12-13-25Z-v0.9.1-rc20-final-4plan`<br/>
リポジトリ SHA: [`592ab48`](https://github.com/libraz/go-oidc-provider/commit/592ab48fa071b39e0f0c8265a49fac5b09940bed)<br/>
...
- **FAILED** — 仕様から挙動が逸脱した状態。現在 **1 failure**（`fapi2-message-signing-id1` の refresh-token DPoP-nonce-retry 経路）— 後述の節を参照。
...
| Module | 内容 |
|---|---|
| `fapi2-security-profile-id2-refresh-token` | OFCS が refresh-token 要求に対して `use_dpop_nonce` 400 を誘発し、付与された nonce を新しい DPoP proof に乗せて同じ要求をリトライする。OFCS のリトライは **同じ** `client_assertion`（同じ `jti`）を再利用する — OP の clientauth 経路はこれを JTI replay と判定し `invalid_client` を返す。 |

FAPI 2.0 仕様は、トークンエンドポイントが `use_dpop_nonce` で一時的に拒否したときの `client_assertion` の `jti` を消費済みと扱うべきかについて明文化していません。OP の現在の解釈は厳格側で、観測した `jti` は要求結果に関わらず消費済みとします。OFCS の期待はより寛容で、`jti` は assertion が状態遷移を成功させたときのみ消費されるべきという読みです。これは既知の継続課題として記録済みで、安全側の修正（`use_dpop_nonce` 400 の直後のリトライに限定して `jti` 再利用を許す）はまだ実装していません。

同じ module は `fapi2-security-profile-id2-test-plan` 上では PASS します — そちらのフローは DPoP nonce リトライ経路に入らないためです。
