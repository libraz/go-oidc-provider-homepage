...
| ID Token の発行と claim | 既定で有効 | [/ja/concepts/tokens](/ja/concepts/tokens) |
...
| refresh token + ローテーション | 既定で有効、長期 refresh は `offline_access` スコープが必要 | [/ja/concepts/refresh-tokens](/ja/concepts/refresh-tokens) |
...
| ID Token 署名アルゴリズム `ES256` / `PS256` | プロファイルが強制 | [/ja/concepts/jose-basics](/ja/concepts/jose-basics) |
...
| refresh token のローテーションと再利用検知 | 既定で有効 | [/ja/concepts/refresh-tokens](/ja/concepts/refresh-tokens) |
...
| token レスポンスの ID Token 署名 | プロファイルが強制 | [/ja/concepts/tokens](/ja/concepts/tokens) |
| request object 署名 (`PS256` / `ES256`) | プロファイルが強制 | [/ja/concepts/fapi](/ja/concepts/fapi) |

### REVIEW と SKIPPED の意味
...
- **FAILED** — 仕様から挙動が逸脱した状態。3 プランを通じて現状 **0 failures**。
...
| `conformance/docker-compose.yml` | OFCS イメージのタグ固定(`release-v5.1.9`)+ JKS truststore 実装 |
...
- **プランスイートのバージョン。** OFCS は `release-v5.1.9` で固定しています。新しい OFCS リリースで追加・改名されたテストは、固定バージョンを引き上げるまで対象外です。
