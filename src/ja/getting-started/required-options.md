...
| [`op.WithCookieKey`](#withcookiekey) | cookie payload の AES-256-GCM 鍵に使う 32 バイトのランダム素材。session と CSRF cookie は署名のみではなく暗号化される。 |
...
issuer は `https://` で始まり、末尾スラッシュ無し、query / fragment 無し、でなければなりません。loopback ホスト（`127.0.0.1`、`[::1]`）は localhost 開発のため `https://` 要件から免除されます。

`internal/discovery.ValidateIssuer` がオプション setter のさらに defense-in-depth として shape を強制します。typo（末尾スラッシュ等）は `op.New` で失敗し、RP が拒否する discovery document を黙って吐くことはありません。
:::
...
- `op.WithLoginFlow(...)` — authenticator + rule を step-up policy（例: password → TOTP、password → N 失敗後 captcha）に組み合わせる。
