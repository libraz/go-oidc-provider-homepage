...
ライブラリは HTTP のライフサイクルやインフラ寄りの論点には踏み込みません。`/metrics` ルートはマウントしませんし、鍵がクラスタ上でどう生きるかにも意見を持ちませんし、tracer も内蔵しません。本セクションでは、OP が公開している差し込み口と、それに対して本番の組み込み側が典型的にどう実装するかを記録します。

## このセクションのページ

| ページ | こういうとき |
|---|---|
| [鍵ローテーション](/ja/operations/key-rotation) | 署名鍵(`Keyset`)と cookie 鍵(`WithCookieKey`)を、生きているセッションを切らずにローテーションする |
...
1. **トランザクショナルクラスタはバックエンドを 1 つに集約します。** composite store は、clients / codes / refresh tokens / access tokens / IATs を 2 つのバックエンドにまたがらせることを拒否します。[hot/cold split](/ja/use-cases/hot-cold-redis) を参照。
2. **揮発サブストアは best-effort です。** Sessions、DPoP nonce、JAR `jti` レジストリは、persistence 無しの Redis でも構いません。eviction は通常運転モードであり、OP は緩い側に倒れる (fail-open)のではなく、ギャップを audit します。[マルチインスタンス展開](/ja/operations/multi-instance) を参照。
