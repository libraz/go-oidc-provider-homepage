...
| `AccessTokens` | 中 | 低優先 | access token TTL(既定 5 分)で自然に回復 |
...
- **PARs と authorization code** — TTL は最長でも 90 秒で、復旧時にはどのみち期限切れです。
...
3. **必要なら**無効化を fan-out します:
   - バックアップが `WithAccessTokenTTL` より古ければ、発行済みの access token はすべて期限切れになっているので、何もする必要はありません。
...
トランザクショナルクラスタ不変条件により、clients / codes / refresh tokens / access tokens / IATs は同じバックエンドを共有しています。クラスタ内の部分復元は SQL レベルの操作(テーブル単位の binlog 復元や PITR)になります。揮発サブストアは独立しているので、`Sessions` を失っても `RefreshTokens` には影響しません。

## Cookie 鍵の損失

cookie 鍵を失う(HSM 破壊、secret manager 抹消など)と、運用中のすべての暗号化済み cookie が無効になります:

- 生きていたブラウザ session は失われます。ユーザは再ログインになります。
- 進行中の consent / interaction フローも失われます。ユーザは最初からやり直しです。
- refresh token には影響しません。cookie 鍵は **session** cookie を封緘するための鍵で、refresh token は封緘していません。
...
- refresh token は opaque 値としては発行可能ですが、紐付く access token は署名できません。
