...
::: warning v0.9.x — `op.WithSPAUI` はまだ未実装
型は将来計画の参照用に公開してありますが、現状で設定すると `op.New` は構成エラーを返します。SPA shell・静的アセットツリー・JSON state 面を 1 オプションで自動マウントするハンドラは v1.0 向けの予約で、まだ着地していません。それまでは `op.WithInteractionDriver(interaction.JSONDriver{})` を渡して JSON state だけ OP に任せ、SPA shell と静的アセットは自前のルーターで配信してください。本ページの構成も以降はこの前提で書いています。
:::
...
- **JSON driver** — プロンプトを HTML ではなく JSON で返す、ライブラリのプラガブル interaction バックエンド。ステートマシンは OP 側に残ります — SPA は `{ kind: "login" | "consent" | ... }` を取得して回答を送り返し、OP が次を決めます。
...
> - [`examples/10-react-login`](https://github.com/libraz/go-oidc-provider/tree/main/examples/10-react-login) — JSON ドライバ上にフル SPA を組み立てた構成。同梱バンドルはビルドステップ無しで動かすための素の HTML/CSS/JS ですが、シーム自体はフレームワーク非依存で、React / Vue / Svelte / Angular いずれも同じ形で乗ります。

## アーキテクチャ

OP は interaction のステートマシンを `/interaction/{uid}` に出します(これはドライバに依らず同じ)。JSON ドライバを選ぶと各プロンプトは JSON で返るので、SPA shell や静的アセットは自前のルーターで好きなパスから配信します。
...
`/authorize` は `/interaction/{uid}` へリダイレクトします。`/authorize` のリダイレクトから RP のコールバックに戻る `code` 付きリダイレクトまでの間は、すべて SPA 上で完結します。
...
`/authorize` は既定で `/interaction/{uid}` へリダイレクトします。先に SPA shell を読み込ませてから(shell が `Accept: application/json` で `/interaction/{uid}` を呼ぶ構成にしたい場合は)、SPA が想定するパス(例: `/login/{uid}`)で SPA shell を配信し、そこから `/interaction/{uid}` を直接 fetch させる構成にしてください。state 面と表示 shell は意図的に分離されており、state は OP、表示は自前のルーターが所有します。
:::

::: info `op.WithSPAUI`(予約)
`op.SPAUI` は v1.0 で `LoginMount` / `ConsentMount` / `LogoutMount` / `StaticDir` を取り、SPA shell・静的アセットツリー・prompt JSON を 1 オプションで自動 mount できる形で着地予定です。現状は前方互換のため型だけ公開していますが、設定すると `op.New` は拒否します。代わりに `interaction.JSONDriver` と自前のルーターを使ってください。
:::
...
`op.WithConsentUI`(独自テンプレートの差し込み)は v1.0 向けの予約で、現状は `op.New` が拒否します。それまでに markup ごと自前で持ちたい場合は、上記の JSON ドライバ経路が動く構成です。
