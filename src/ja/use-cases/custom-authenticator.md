...
ライブラリの初期は `Authenticator` を直接公開していました。step-up フローを書く組み込み側は、authenticator の `Begin` の中で「次にどの factor を走らせるか」を再実装することになり、これはオーケストレータの不変条件「1 factor = 1 ステップ」に反していました。表面を:
...
…に分けたことで、オーケストレータが順序と重複排除を所有し、自前コードは factor の仕組みを所有できるようになりました。`ExternalStep` は、組み込み `Step` のいずれにも嵌らない `Authenticator` を橋渡しします。
