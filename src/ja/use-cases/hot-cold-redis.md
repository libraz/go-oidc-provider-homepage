...
> **ソース:**
> - [`examples/08-composite-hot-cold`](https://github.com/libraz/go-oidc-provider/tree/main/examples/08-composite-hot-cold) — 開発用 SQL + inmem。
> - [`examples/09-redis-volatile`](https://github.com/libraz/go-oidc-provider/tree/main/examples/09-redis-volatile) — 本番用 MySQL + Redis。
...
volatile, err := redis.Open(redis.Options{
  Addr:     "rediss://redis:6380",  // <-- デフォルトで TLS 必須
...
`redis.Open` は TLS（`rediss://`）と AUTH 無しでは **起動を拒否** します。refresh token chain を平文で流す構成を出荷させないためです。例外口 `redis.WithDevModeAllowPlaintext()` は `examples/` 実行とローカル開発のためだけにあります — 本番で使うのは「手で打ち込まないと出てこない」セキュリティ後退の選択肢です。
:::
...
| `SessionStore` | デフォルト永続 (SQL)、`WithSessionDurabilityPosture` で揮発にオプトイン |
| `InteractionStore` | volatile (Redis) |
| `ConsumedJTIStore` | volatile (Redis) |
| `PARStore` | volatile (Redis) |

::: info 新しいサブストアが durable 側に残る理由
`OpaqueAccessTokenStore` と `GrantRevocationStore` はトランザクションクラスタの一部です — 起点となる grant / refresh の書き込みと同時にコミットされる必要があります。Redis アダプタはどちらのアクセサからも `nil` を返すので、composite アダプタは両者を非トランザクションのバックエンドに振り分けることができません。これらのサブストアが必要な組み込み側は durable 側に SQL を配置してください。opaque AT を有効化せず、サーバ側の JWT 失効処理も必要としない Redis 専用デプロイであれば、両者とも `nil` のままで構いません。
:::

::: details SessionStore がどちらにもなる理由
揮発セッションストア（メモリ圧で追い出される、複製保証無し）は、多くのデプロイで許容範囲です — 最悪ケースでもユーザの再認証で済むためです。一方で、再起動を跨いでログイン状態を保ちたい組み込み側もいます。ライブラリは `op.WithSessionDurabilityPosture(SessionDurabilityVolatile | SessionDurabilityDurable)` で選択肢を公開しており、その値を back-channel logout 監査イベントに伝播します。これにより SOC ダッシュボードは「揮発配置における想定内のギャップ」と「永続配置における想定外のギャップ」を区別できます。
:::
