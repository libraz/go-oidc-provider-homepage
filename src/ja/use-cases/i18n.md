...
> **ソース:** [`examples/15-i18n-locale`](https://github.com/libraz/go-oidc-provider/tree/main/examples/15-i18n-locale) — `:8080` の listener を起動する前に、in-process の httptest サーバで優先順序の各行を駆動する self-verify probe を実行します。ブラウザを使わずとも example の出力からリゾルバの挙動が確認できます。
