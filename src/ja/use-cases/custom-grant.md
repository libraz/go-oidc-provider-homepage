...
標準 grant が dispatcher に識別された後で共通して通る処理パスのことです。クライアントの許可リストとの scope の積集合、登録 resource との audience の積集合、グローバル上限による TTL の頭打ち、sender-bound トークンへの `cnf` の押印、リフレッシュトークンの lineage 追跡などが含まれます。custom grant は薄く切り取ったスライス(scope / audience / TTL / `cnf`)だけを共有し、リフレッシュトークンの lineage は共有しません。後述の「OP が拒否すること」を参照してください。
:::
...
- **リフレッシュトークン**。`CustomGrantResponse.RefreshToken` を非空で返すと `server_error` に集約されます。handler 発行リフレッシュトークンの lineage 永続化と rotation は v2+ の設計課題。それまでは空のままにしてください。同梱の token-exchange handler だけは grant_type URN がゲート前にチェックされるため例外

::: details リフレッシュトークンの lineage とは
OP はリフレッシュトークンごとに親を記録するため、ローテーションは chain（`A → B → C`）を作ります。そのうちの 1 本が再利用された場合（RFC 9700 §2.2.2）、OP は子孫すべてを一括で失効させられます。custom grant はこの仕組みから外れており、handler が trace の起点を選ぶ以上、OP は自分が作っていない chain について推論できません。handler 発行のリフレッシュトークンに lineage シームができるまで、これを返すのは hard error です。
:::
...
組み込み側が `urn:example:libraz:service-token-exchange` を定義、OP が `op.WithCustomGrant` 経由でルーティング、handler が `BoundAccessToken` を返し、リクエストの DPoP / mTLS confirmation に紐付いた JWT アクセストークンを dispatcher が発行します。ファイル: `op.go`(OP の組み立て + handler)、`client.go`(client 側)、`probe.go`(self-verify)。
