...
OIDC / OAuth 2.0 が想定する基本シナリオは **第三者** ケースです: 外部の RP（どこかの SaaS）が、あなたの OP が保持するユーザデータへのアクセスを欲しがる。だから OIDC Core 1.0 §3.1.2.4 は「あなたは SaaS-X にあなたのメールを読む権限を委譲しますか？」と明示的に問う同意プロンプトを必須化しています。
