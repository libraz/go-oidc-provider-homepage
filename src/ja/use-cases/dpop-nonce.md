...
**DPoP**（"Demonstrating Proof of Possession", RFC 9449）は、access token をクライアントが保持する鍵に紐づける仕組みです。クライアントは API 呼び出しのたびに、その鍵で署名した新しい JWT を `DPoP:` ヘッダで提示し、「私はこのトークンを発行されたクライアントと同一です」と証明します。漏洩した DPoP-bound トークンは、鍵を持たない攻撃者にとって無価値です。
...
::: details 用語の補足
- **DPoP proof** — クライアントがリクエスト毎に署名する小さな JWT。「access token がバインドされた秘密鍵を、いまも自分が保持している」ことを示します。基本は [送信者制約](/ja/concepts/sender-constraint) を参照。
