---
title: カスタム同意 UI
description: op.WithConsentUI は v1.0 向けの予約。現バージョンではロケール bundle 上書きか JSON ドライバで同意画面をカスタマイズします。
---

# ユースケース — カスタム同意 UI

::: warning v0.9.x — `op.WithConsentUI` はまだ未実装
型は将来計画の参照用に公開してありますが、現状で設定すると `op.New` は構成エラーを返します。組み込み側のテンプレートを描画するハンドラは v1.0 向けの予約で、まだ着地していません。当面は以下の 2 経路のいずれかを使ってください。
:::
...
OP 同梱の同意画面でもひと通り動きますが、ほぼ確実に独自ブランド(ロゴ・コピー・プライバシー / 利用規約リンク・i18n)に差し替えたくなります。`op.WithConsentUI` が着地するまでは、ロケール bundle 上書き(小さな変更向け)か JSON ドライバ(SPA でフル制御)が動く経路です。
...
## 経路 2 — JSON ドライバ(markup を完全に自前で持つ)

レイアウト・ブランドアセット・フレームワーク描画など、markup ごと自前で持ちたいケースでは JSON ドライバに切り替えます。OP は同意プロンプトを `{ type: "consent.scope", data: { scopes: [...] }, csrf_token, ... }` という JSON で返し、自前のページ(または SPA)がそれを描画してユーザの選択を POST で送り返します。
...
経路 1 は同梱 CSP(`default-src 'none'; style-src 'unsafe-inline'`)と描画される HTML をそのまま使います。経路 2 は自前のページが必要な CSP を自分で設定します。

## `op.WithConsentUI` が着地したら(v1.0)

予約済みのオプションは、組み込み側の `*html/template.Template` と正規のコンテキスト(`Client` / `Scopes` / `CSRFToken` / `User`)を受け取る形で v1.0 に着地します。ハンドラが組み込まれるまではテンプレートが一切実行されないため、組み込み側が「テンプレートが描画されている」と誤認しないよう、`op.New` は構築時に拒否します。これまで `WithConsentUI` を使っていたページは、当面は上記 2 経路のどちらかを採用し、v1.0 着地時に再検討してください。
