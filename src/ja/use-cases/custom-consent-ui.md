...
OP 同梱の同意画面でもひと通り動きますが、ほぼ確実に独自ブランド(ロゴ・コピー・プライバシー / 利用規約リンク・i18n)に差し替えたくなります。小さな文言差し替えはロケール bundle、サーバー描画の独自 HTML は `op.WithConsentUI`、SPA での完全制御は JSON ドライバが向いています。
...
## 経路 2 — カスタム同意 template(サーバー描画)

ブランド付き HTML を持ちつつ、state・CSRF・同意の永続化は OP に任せたい場合は、parse 済みの `*html/template.Template` を `op.WithConsentUI` で渡します。template には `interaction.ConsentTemplateData` が渡され、`Client`、`Scopes`、`StateRef`、`CSRFToken`、`ApprovedScopesField`、`SubmitMethod`、`SubmitAction` を参照できます。
...
## 経路 3 — JSON ドライバ(markup を完全に自前で持つ)
...
経路 1 は同梱 CSP(`default-src 'none'; style-src 'unsafe-inline'`)と描画される HTML をそのまま使います。経路 2 は自前 template を OP の HTML driver に差し込みます。経路 3 は自前のページが必要な CSP を自分で設定します。
