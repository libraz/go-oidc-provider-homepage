...
::: warning v0.9.x — chooser テンプレートのオプションはまだ未実装
以降で説明するセッションマネージャ API と、`prompt=select_account` の同梱 interaction は今のリリースでも動きます。ただし、`op.WithChooserUI` でテンプレートを差し替える経路は v1.0 向けの予約で、現状で設定すると `op.New` は構成エラーを返します。ランタイムのマウントが着地するまでは、このオプションは設定せず、同梱の chooser テンプレートをそのまま使ってください。
:::
...
> **ソース:** [`examples/13-multi-account`](https://github.com/libraz/go-oidc-provider/tree/main/examples/13-multi-account)
