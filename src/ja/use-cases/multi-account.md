...
::: details 用語の補足
- **`prompt` パラメータ** — RP が `/authorize` に乗せて、OP に UI の出し方を指示するヒントです: `none`（UI を出さず、既存セッションを返すか失敗）、`login`（再ログイン強制）、`consent`（同意プロンプト強制）、`select_account`（アカウント選択 UI）。空白区切りで複数指定できます。
