...
| event 定数 | 発火タイミング | 想定シビアリティ | 関連ページ |
|---|---|---|---|
| `AuditConsentGranted` | ユーザがプロンプトで同意した | info | [ガイド: consent](/ja/concepts/consent) |
| `AuditConsentGrantedFirstParty` | ファーストパーティの自動同意を適用した(プロンプトなし) | info | [ユースケース: ファーストパーティ consent](/ja/use-cases/first-party) |
| `AuditConsentGrantedDelta` | delta consent が、新規センシティブ scope の再プロンプトを発生させた | info | [ガイド: consent](/ja/concepts/consent) |
