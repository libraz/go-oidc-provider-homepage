...
`op.WithProfile(profile.FAPI2Baseline)` does the whole thing — auto-enables PAR + JAR + DPoP, intersects `token_endpoint_auth_methods_supported` with the FAPI allow-list, locks the algorithm list.
